import { checkTooltip, expectPairInSection, scrollToEl, typeIntoInput, waitForAppReady } from '../helpers/app';
...
describe('Aliases', () => {
  ...
  it('can add an alias', async () => {
    ...
    await typeIntoInput(labelInput, 'test_label');

    const idInput = await byTestId('alias_id_input');
    await typeIntoInput(idInput, 'test_id');
